...
class Bod extends CI_Controller {
  ...
  public function list_po_approval_bod()
  {
  	$this->load->view('bod/list_po_approval_bod');
  }

  public function list_po_approved_bod()
  {
  	$this->load->view('bod/list_po_approved_bod');
  }
  ...
  public function get_po_approval_bod()
  {
    $sql="SELECT * FROM db_demo_pms.po_master pm
          ...
          where pm.status_po='approved' and pm.status_bod='open' or pm.status_ob='open'";
	...
	public function get_po_approved_bod()
	{
		$sql="SELECT * FROM db_demo_pms.po_master pm
					JOIN db_pembelian.po_supplier ps on ps.supplier_code=pm.code_supplier
					where pm.status_po='approved' and pm.status_bod='approved' || pm.status_ob='approved'";
...
{
  $sql_table="SELECT nama_pasar, pd.last_hgs, pm.cur_id, pd.ppn, pd.pph, dm.basic_price_budget, pmd.qty_angka, pmd.qty_tot_buy, dm.nama_bahan, sum(qty_buy) as 'tot_qty_buy',dm.satuan, sum(qty_po) as 'tot_qty_po', stn_po, hgs, disc, sum(jumlah) as 'jumlah' FROM po_detail pd
    join po_master pm on pm.id_po_master=pd.po_master_id
...
{
	$sql='update db_pembelian.po_parameter set item_max="'.str_replace(',','',$this->input->post('item')).'",total_max="'.str_replace(',','', $this->input->post('total')).'" where id_parameter=1';
	// echo $sql;
	$this->db->query($sql);
