        ...
        <div class="col-md-12">
          ...
          <div class="portlet green-jungle box full-height-content full-height-content-scrollable">
										...
										<table class="table table-striped table-bordered table-hover" id="table_material">
										 <thead>
													<tr>
														...
														<!-- <th> Kode Barang </th> -->
														<!-- <th> Nama Barang</th> -->
														<th> Nama Barang <span class="required" aria-required="true"> * </span> </th>
														<th> Merek </th>
														<!-- <th> SPPr NO</th> -->
                ...
                <!-- /span -->
                    ...
                    <div class="col-md-6 ">
                      ...
                      <div class="form-group hide">
                        ...
                        <div class="col-md-9">
                          <input   name="id_po" type="text" class="form-control" placeholder="PO No">
                          <span class="help-block">
                          </span>
                        </div>
                      </div>
            ...
            <div class="modal-footer">
              ...
              <button id="submit-first" type="button" onclick="save_po_master()" class="btn green">Submit</button>
            </div>
          </div>
							...
							<div class="row">
								<div class="col-md-12 hide">
									<div class="form-group">
										...
										<div class="col-md-9">
											<input   name="id_po_modal" type="text" class="form-control" placeholder="PO No">
											<span class="help-block">
											</span>
										</div>
									</div>
...
function save_po_bum()
{
	...
	$.ajax({
			...
			success: function(data)
			{
				if(data.status)
				{
						var tr='<tr> <td>CIA-BUM</td> <input type="hidden" name="type_top[]" value="'+data.id+'">';
...
function save_po_cod()
{
	...
	$.ajax({
			...
			success: function(data)
			{
				if(data.status)
				{
						var tr='<tr> <td>COD</td><input type="hidden" name="type_top_po_top[]" value="'+data.id+'">';
...
function save_po_credit()
{
	...
	$.ajax({
			...
			success: function(data)
			{
				if(data.status)
				{
						var tr='<tr> <td> Credit-'+data.data.type_top+'</td><input type="hidden" name="type_top_po_top[]" value="'+data.id+'>"</td>';
						tr+='<td> '+data.data.description+'</td>';
						tr+='<td> '+numeral(data.data.nominal).format(0,0)+'</td>';
						tr+='<td> '+data.data.time+'</td>';
						tr+='<td> <a class="btn btn-danger btn-delete" onclick="delete_top('+data.id+')"><i class="fa fa-trash"></i></a></td>';
...
function save_po_process()
{
	...
	$.ajax({
			...
			success: function(data)
			{
				if(data.status)
				{
						var tr='<tr> <td>'+data.data.type_pp+'</td><input type="hidden" name="type_top_po_dp[]" value="'+data.id+'">';
						tr+='<td> '+data.data.description+'</td>';
...
function save_po_master()
{
	...
	$.ajax({
			url : '<?php echo site_url().'/po/save_po_other'?>',
			type: "POST",
			// data: {'id_item': id,'colom':colom, 'table': table},
			data: formd,
			processData: false,
...
{
	var string_td='<tr>';
	string_td+=' <td><input type="hidden" value="'+count+'"name="num_row[]"><input type="text" name="'+count+'_nama_pasar"></td>\
