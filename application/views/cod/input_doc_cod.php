									...
									<div class="row">
										<div class="col-md-6 hide">
											...
											<div class="form-group">
												...
												<div class="col-md-9">
													<input name="type_top" type="text" value="cod" class="form-control" placeholder="">
													<span class="help-block">
													 </span>
												</div>
											</div>
											<div class="form-group">
												<label class="control-label col-md-3">ID PJUM</label>
												<div class="col-md-9">
													<input name="id_pjum" type="text" value="<?php echo $id_pjum;?>" class="form-control" placeholder="">
													<span class="help-block">
													 </span>
												</div>
											</div>
										...
										<div class="col-md-6 ">
											...
											<div class="form-group">
												<label class="control-label col-md-3">Nominal Invoice<span class="required" aria-required="true"> * </span></label>
												<div class="col-md-9">
													<input name="invoice_value" value="<?php echo $sum; ?>" type="text" class="form-control" placeholder="">
													<span class="help-block">
													 </span>
												</div>
											</div>
                      ...
                      <div class="form-group">
                        ...
                        <div class="col-md-9">
                          ...
                          <span class="help-block">
										...
										<div class="col-md-6 ">
											<div class="form-group">
												<label class="control-label col-md-3">Doc Other NO</label>
												<div class="col-md-9">
											...
											<div class="form-group">
												<label class="control-label col-md-3">Date Doc Other</label>
												<div class="col-md-9">
                      ...
                      <div class="form-group">
                        ...
                        <div class="col-md-9">
                          ...
                          <span class="help-block">
										...
										<div class="col-md-6 ">
											<div class="form-group">
												<label class="control-label col-md-3">Doc other</label>
												<div class="col-md-9">
											...
											<tbody>
												...
												for ($i=0; $i < count($files) ; $i++) {
													foreach ($files[$i] as $key => $value) {
														$isi="'".$key."','".$value."'";
														echo '<tr> <td>'.$key.'</td>';
														echo '<td> <a download href="'.base_url().'uploads/pjum/credit/'.$id_pjum.'/'.$key.'/'.$value.'">'.$value.'</a></td>';
							...
							<div class="row">
								...
								<div class="col-md-6">
									<div class="row">
										...
										if($id_pjum!=0){
											echo '<button id="submit-all" onclick="sumbit_po()" type="submit" class="btn green">Submit</button>';
										}
									}
										?>
											<button type="button" onclick="goBack()" class="btn default">Cancel</button>
...
jQuery(document).ready(function() {
  ...
  var id_po=$("[name='id_po']").val();
	...
	var url='<?php echo site_url().'bum/get_list_credit_by_id/'?>'+'<?php echo $id_top; ?>';
...
function save_bjum_master()
{
	...
	$.ajax({
			url : '<?php echo site_url().'pjum/save_pjum/1'?>',
			type: "POST",
				...
				if(data.status)
				{
					...
					// alert('success');
						bootbox.alert({
							...
							message: 'Well Done !!!!',
							callback: function(){
								var url='<?php echo site_url().'pjum/input_doc_cod/'.$id_po.'/'.$id_top.'/'?>'+data.id_pjum;
								window.location=url;
								// location.reload();
							}
						});
...
function sumbit_po()
{
	...
	var formd = new FormData($("#form-pjum")[0]); //get data
	$.ajax({
			url : '<?php echo site_url().'/pjum/sent_pjum'?>',
			type: "POST",
			// data: {'id_item': id,'colom':colom, 'table': table},
			data: formd,
			processData: false,
				...
				if(data.status)
				{
					...
					// alert('success');
						bootbox.alert({
							...
							message: 'Well Done !!!!',
							callback: function(){
								var url='<?php echo site_url().'pjum/list_pjum_approval_c/'?>';
								window.location=url;
							}
						});
				 }else{
...
function get_data(){
	Metronic.startPageLoading({animate: true});
	var url='<?php echo site_url().'/pjum/get_data_pjum/'?>'+"<?php echo $id_pjum;?>";
	$.ajax({
			url : url,
			type: "POST",
			...
			{
				Metronic.stopPageLoading();
				var col=['doc_no', 'doc_date', 'invoice_no','invoice_value', 'invoice_date', 'sj_no', 'sj_date', 'fp_no', 'fp_date', 'ttbp_no', 'ttbp_date'];
...
function delete_item(folder, file){
	var id_top=$('[name="id_pjum"]').val();
	console.log(id_top+' allala');
	$.ajax({
			url : '<?php echo site_url().'/pjum/delete_pjum'?>',
			type: "POST",
			data: {'id': id_top, 'type':'credit', 'folder':folder, 'file': file},
			// data: fo,
			// processData: false,
			// contentType: false,
			dataType: "JSON",
...
function approve()
{
	bootbox.confirm({
		...
		callback: function (result) {
				if(result){
					Metronic.startPageLoading({animate: true});
					var url='<?php echo site_url().'/pjum/approve_pjum/'?>'+'<?php if(isset($status)) echo $id_pjum ?>';
					$.ajax({
							url : url,
							type: "POST",
							// data: {'id_bpmb': id_bpmb},
							...
							success: function(data)
							{
								...
								bootbox.alert({
									...
									message: 'Well Done !!!!',
									callback: function(){
										var url='<?php echo site_url().'pjum/list_pjum_approved_c/'?>';
										window.location=url;
									}
								});
							},
