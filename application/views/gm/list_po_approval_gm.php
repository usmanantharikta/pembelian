...
<div class="modal fade" id="comment" role="dialog">
	<div class="modal-dialog">
		...
		<div class="modal-content">
			...
			<div class="modal-body">
				<form id="comment_form" method="post" enctype="multipart/form-data" class="form-horizontal">
						...
						<div class="row">
							<div class="col-md-12 hide">
								<div class="form-group">
									...
									<div class="col-md-9">
										<input name="id_po" type="text" class="form-control" value="" placeholder="">
							...
							<div class="col-md-12">
								<div class="form-group">
									...
									<div class="col-md-9">
										<textarea name="comment"  class="js-data-example-ajax form-control"></textarea>
									</div>
								</div>
							</div>
							<!--/span-->
			...
			<div class="modal-footer">
				...
				<button type="button" class="btn btn-success" onclick="save_comment()">Save</button>
			</div>
		</div>
...
jQuery(document).ready(function() {
	...
	//active slide bar
	$('#gm').addClass('active open');
	$('#list_gm_approval').parent().addClass('active');
...
function initial_table()
{
	...
	$('.foot-form').addClass('hide');
		//dataTables material
	var url='<?php echo site_url().'gm/get_po_approval_bod/'?>';
...
function show_po(id_po_master)
{
	...
	$.ajax({
			url : '<?php echo site_url().'bod/view_po/'?>'+id_po_master,
			type: "POST",
			// data: {'pr_no': pr_no},
			// processData: false,
			// contentType: false,
        ...
        $('.tot_before').text(data.master.cur_id+' '+numeral(data.tot_harga).format(0,0));
        $('.m_ppn').text(data.master.cur_id+' '+numeral(data.master.ppn).format(0,0));
        $('.m_tot_after').text(data.master.cur_id+' '+numeral(data.master.total).format(0,0));
        $('.m_pph').text(data.master.cur_id+' '+numeral(data.master.pph).format(0,0));
        ...
        //
				...
				$(".over").parent().parent().addClass('danger');
...
function approve_bod(id_po_master)
{
  if(id_po_master==0){
	...
	bootbox.confirm({
		message: "Are you sure want approve this PO ?",
		buttons: {
				confirm: {
						label: 'Yes',
		...
		callback: function (result) {
				if(result){
					Metronic.startPageLoading({animate: true});
					var url='<?php echo site_url().'/gm/approve_po_gm/'?>'+id_po_master;
					$.ajax({
							url : url,
							type: "POST",
...
function save_comment()
{
	...
	$(".help-block").empty();
	var formd = new FormData($("#comment_form")[0]); //get data
	$.ajax({
			url : '<?php echo site_url().'bod/save_comment'?>',
			type: "POST",
			data: formd,
			processData: false,
			contentType: false,
