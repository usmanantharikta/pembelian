...
<head>
<meta charset="utf-8"/>
<title>List PJUM Approval</title>
...
<div class="page-container">
	<!-- BEGIN SIDEBAR -->
<?php $this->load->view('include/slidebar_keuangan');?>
	<!-- END SIDEBAR -->
	<!-- BEGIN CONTENT -->
	<div class="page-content-wrapper">
		<div class="page-content">
			<?php
			if(isset($_SESSION['username']) && $_SESSION['division']=='keuangan'){
				?>
			<!-- BEGIN PAGE HEADER-->
			<h3 class="page-title">
			...
			<div class="page-bar">
				<ul class="page-breadcrumb">
					...
					<li>
						<span>List PJUM COD Approval</span>
					</li>
				</ul>
			</div>
			<!-- END PAGE HEADER-->
			...
			<div class="col-md-12 dkp-isian ">
				...
				<div class="portlet red-flamingo box full-height-content full-height-content-scrollable">
					<div class="portlet-title">
						<div class="caption">
							<i class="fa fa-shopping-cart"></i>List COD / Dana Taktis
						</div>
						<div class="tools">
						...
						<form id="form_cod" method="post" enctype="multipart/form-data" class="form-horizontal">
							  <div class="form-body">
									<table class="table table-striped table-bordered table-hover" id="table_cod">
									 <thead>
												<tr>
													<th class="table-checkbox">Roll NO</th>
														<th> Date Roll</th>
														<th> Total </th>
														<th> Create BY</th>
														<th> Create Date </th>
...
jQuery(document).ready(function() {
	...
	$('#list_cod_r').parent().addClass('active');
...
function initial_table()
{
	...
	$('.foot-form').addClass('hide');
		//dataTables material
	var url='<?php echo site_url().'keuangan/get_list_cod_roll/'?>';
...
// build po ya, males rubah nama fungsi
 function build_cod()
 {
	 ...
	 $.ajax({
			 url : '<?php echo site_url().'bum/build_cod'?>', //nih yang bener
			 type: "POST",
			 ...
			 success: function(data)
			 {
				 if(data.status){
				 Metronic.stopPageLoading();
				 var url='<?php echo site_url().'bum/process_cod/'?>';
 ...
 function ack(id){
	bootbox.confirm({
		...
		callback: function (result) {
				if(result){
					Metronic.startPageLoading({animate: true});
					var url='<?php echo site_url().'/keuangan/approve_cod/'?>'+id;
